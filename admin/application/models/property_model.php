...
{
	//property
	public function createproperty($name,$area,$description,$lat,$long,$price,$builder,$propertytype,$status)
	{
	...
	public function editproperty( $id,$name,$area,$description,$lat,$long,$price,$builder,$propertytype,$status)
	{
		$data = array(
			'name' => $name,
			'area' => $area,
	...
	function searchproperty($locality,$city,$propertytype)
	{
        ...
        $query=$this->db->query("SELECT `property`.`id`,`property`.`name` as `property`,`propertyinfo`.`bedroom` as `bedroom`,`property`.`price`,`builder`.`name` as `builder`,`builderimage`.`image` as `builderimage`,`propertyinfo`.`propertyarea`,`propertyimage`.`image`,`city`.`name` as `city`,`area`.`name` as `locality`,`propertytype`.`name` as `propertytype` FROM `property`
		INNER JOIN `builder` ON `builder`.`id`=`property`.`builder`
	...
	{
        $query['property']=$this->db->query("SELECT `property`.`id`,`property`.`name` as `property`,`property`.`price`,`builder`.`contact` as `buildercontact`,`builder`.`website` as `builderwebsite`,`builder`.`email` as `builderemail`,`builder`.`name` as `builder`,`propertyinfo`.`propertyarea`,`propertyinfo`.`propertyaddress`,`propertyinfo`.`propertywalkthrough`,`propertyinfo`.`propertyspecialoffers`,`propertyinfo`.`propertyage`,`propertyinfo`.`bedroom`,`propertyinfo`.`bathroom`,`propertyinfo`.`kitchen`,`propertyinfo`.`floor`,`propertyinfo`.`presentuse`,`propertyinfo`.`nearby`,`property`.`lat`,`property`.`long`,`property`.`description`,`city`.`name` as `city`,`state`.`name` as `state`,`area`.`name` as `area`,`propertyinfo`.`possession`,`propertyinfo`.`propertypanaroma`,`propertyinfo`.`bankloan`,`propertyinfo`.`buildings`,`builderimage`.`image` as `builderimage`,`propertyinfo`.`propertyage`,`property`.`builder`,`builder`.`name` as `buildername`,`builder`.`description` as `builderdesc`,`area`.`city` as `cityid` FROM `property`
		INNER JOIN `builder` ON `builder`.`id`=`property`.`builder` 
